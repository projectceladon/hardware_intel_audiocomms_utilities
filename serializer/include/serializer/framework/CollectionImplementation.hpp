...
/** Specialize SerializerChildren to patern match Collection serialization.
 *
 * @see CollectionTrait for argument documentation.
 */
template <const char *_tag, class Collection, class ItemTrait, bool takeOwnership>
class SerializerChildren<CollectionTrait<_tag, Collection, ItemTrait, takeOwnership> >
{
private:
...
public:
                        ...
                        TiXmlNode &xmlCollection)
    {
        ...
        for (ConstIterator it = collection.begin(); it != collection.end(); it++) {
            ...
            Result res = Serializer<ItemNbTrait>::toXml(itemNb, xmlItemNb,
    ...
    static Result fromXml(const TiXmlNode &xmlCollection, Collection &collection)
    {
        ...
        while ((xmlItemNb = xmlCollection.FirstChild(formatIndex(nb).c_str())) != NULL) {
            Item item;
            ItemNb itemNb(item);
            Result res = Serializer<ItemNbTrait>::fromXml(*xmlItemNb, itemNb,
                                                          formatIndex(nb).c_str());
            if (res.isFailure()) {
                return res << " (While deserializing collection item" << nb << ")";
            }
